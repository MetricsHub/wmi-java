...
class WqlQueryTest {
	...
	void testBuildSelectedProperties() {
		...
		assertEquals(
			Arrays.asList("propa", "propb", "propc"),
			WqlQuery.buildSelectedProperties("   propA  ,  propB,propC   ")
		);
	}
	...
	void testNewInstance() throws Exception {
		...
		assertThrows(
			...
			() -> WqlQuery.newInstance("SELECT select __PATH,    DNSHostName   FROM      Win32_ComputerSystem")
		);
		assertThrows(WqlQuerySyntaxException.class, () -> WqlQuery.newInstance("SELECT* FROM Win32_ComputerSystem"));
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("SELECT __PATH,DNSHost-Name,SystemType FROM Win32_ComputerSystem")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("SELECT __PATH,DNSHostName,,SystemType FROM Win32_ComputerSystem")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("SELECT __PATH,DNSHostName, ,SystemType FROM Win32_ComputerSystem")
		...
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("select__PATH,DNSHostName,SystemType from Win32_ComputerSystem")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("select DNSHostName,SystemType fromWin32_ComputerSystem")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("select DNSHostName,SystemTypefrom Win32_ComputerSystem")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("select *, Model,Manufacturer from where Name = \"PC14\"")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("select Model,Manufacturer where Name = \"PC14\"")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() ->
		...
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("  SELECT  ASSOCIATORS OF\n{\tWin32_Process.ProcessId=1\n}")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("SELECT Temperature, FROM ASSOCIATORS OF { Win32_Process.ProcessId=1 }")
		);
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("SELECT Temperature, * FROM ASSOCIATORS OF { Win32_Process.ProcessId=1 }")
		...
		assertThrows(
			WqlQuerySyntaxException.class,
			() -> WqlQuery.newInstance("SELECT Temperature,,TemperatureMax FROM ASSOCIATORS OF { Win32_Process.ProcessId=1 }")
		);
		...
		{
			WqlQuery wql = WqlQuery.newInstance(
				"select    __PATH   ,    SystemType,    DNSHostName  from    Win32_ComputerSystem"
			);
		...
		{
			WqlQuery wql = WqlQuery.newInstance(
				"SELECT DriveInfo.Name,DriveInfo.NumberPaths,DriveInfo.SerialNumber FROM MPIO_DISK_INFO WHERE condition"
			);
			assertEquals(
				Arrays.asList("driveinfo.name", "driveinfo.numberpaths", "driveinfo.serialnumber"),
				wql.getSelectedProperties()
			);
		...
		{
			WqlQuery wql = WqlQuery.newInstance(
				"SELECT Temperature.Current, Temperature.Max FROM ASSOCIATORS OF {Win32_Process.ProcessId=1}"
			);
			...
			assertEquals(
				1,
				propertiesMap.size(),
				"Temperature.Current and Temperature.Max must have been merged under Temperature"
			);
