...
import com.sun.jna.platform.win32.COM.Wbemcli.IWbemContext;
...
import org.metricshub.wmi.AutoCloseableReadWriteLock;
import org.metricshub.wmi.Utils;
import org.metricshub.wmi.WmiHelper;
import org.metricshub.wmi.WqlQuery;
import org.metricshub.wmi.exceptions.WmiComException;
import org.metricshub.wmi.exceptions.WqlQuerySyntaxException;
import org.metricshub.wmi.remotecommand.RemoteProcess;
import org.metricshub.wmi.windows.remote.WindowsRemoteCommandResult;
...
public class WmiWbemServices implements WindowsRemoteExecutor {
	...
	static {
		...
		map.put(
			2147749892L,
			"WBEM_E_PROVIDER_FAILURE: Provider has failed at some time other than during initialization. (0x80041004)"
		);
		...
		map.put(
			2147749897L,
			"WBEM_E_NOT_AVAILABLE: Resource, typically a remote server, is not currently available. (0x80041009)"
		);
		map.put(
			2147749898L,
			"WBEM_E_CRITICAL_ERROR: Internal, critical, and unexpected error occurred. Report the error to Microsoft Technical Support. (0x8004100A)"
		);
		map.put(
			2147749899L,
			"WBEM_E_INVALID_STREAM: One or more network packets were corrupted during a remote session. (0x8004100B)"
		);
		...
		map.put(
			2147749905L,
			"WBEM_E_PROVIDER_NOT_FOUND: Provider referenced in the schema does not have a corresponding registration. (0x80041011)"
		);
		map.put(
			2147749906L,
			"WBEM_E_INVALID_PROVIDER_REGISTRATION: Provider referenced in the schema has an incorrect or incomplete registration. (0x80041012)"
		);
		map.put(
			2147749907L,
			"WBEM_E_PROVIDER_LOAD_FAILURE: COM cannot locate a provider referenced in the schema. (0x80041013)"
		);
		map.put(
			2147749908L,
			"WBEM_E_INITIALIZATION_FAILURE: Component, such as a provider, failed to initialize for internal reasons. (0x80041014)"
		);
		map.put(
			2147749909L,
			"WBEM_E_TRANSPORT_FAILURE: Networking error that prevents normal operation has occurred. (0x80041015)"
		);
		map.put(
			2147749910L,
			"WBEM_E_INVALID_OPERATION: Requested operation is not valid. This error usually applies to invalid attempts to delete classes or properties. (0x80041016)"
		);
		...
		map.put(
			2147749913L,
			"WBEM_E_ALREADY_EXISTS: In a put operation, the wbemChangeFlagCreateOnly flag was specified, but the instance already exists. (0x80041019)"
		);
		map.put(
			2147749914L,
			"WBEM_E_OVERRIDE_NOT_ALLOWED: Not possible to perform the add operation on this qualifier because the owning object does not permit overrides. (0x8004101A)"
		);
		map.put(
			2147749915L,
			"WBEM_E_PROPAGATED_QUALIFIER: User attempted to delete a qualifier that was not owned. The qualifier was inherited from a parent class. (0x8004101B)"
		);
		map.put(
			2147749916L,
			"WBEM_E_PROPAGATED_PROPERTY: User attempted to delete a property that was not owned. The property was inherited from a parent class. (0x8004101C)"
		);
		map.put(
			2147749917L,
			"WBEM_E_UNEXPECTED: Client made an unexpected and illegal sequence of calls, such as calling EndEnumeration before calling BeginEnumeration. (0x8004101D)"
		);
		map.put(
			2147749918L,
			"WBEM_E_ILLEGAL_OPERATION: User requested an illegal operation, such as spawning a class from an instance. (0x8004101E)"
		);
		map.put(
			2147749919L,
			"WBEM_E_CANNOT_BE_KEY: Illegal attempt to specify a key qualifier on a" +
			" property that cannot be a key. The keys are specified in the class " +
			"definition for an object and cannot be altered on a per-instance basis. " +
			"(0x8004101F)"
		);
		map.put(
			2147749920L,
			"WBEM_E_INCOMPLETE_CLASS: Current object is not a valid class definition. " +
			"Either it is incomplete or it has not been registered with WMI using SWbemObject." +
			"Put_. (0x80041020)"
		);
		...
		map.put(
			2147749924L,
			"WBEM_E_PROVIDER_NOT_CAPABLE: Provider cannot perform the requested operation. " +
			"This can include a query that is too complex, retrieving an instance, creating or " +
			"updating a class, deleting a class, or enumerating a class. (0x80041024)"
		);
		map.put(
			2147749925L,
			"WBEM_E_CLASS_HAS_CHILDREN: Attempt was made to make a change that invalidates a subclass. (0x80041025)"
		);
		map.put(
			2147749926L,
			"WBEM_E_CLASS_HAS_INSTANCES: Attempt was made to delete or modify a class that has instances. (0x80041026)"
		);
		map.put(2147749927L, "WBEM_E_QUERY_NOT_IMPLEMENTED: Reserved for future use. (0x80041027)");
		map.put(
			2147749928L,
			"WBEM_E_ILLEGAL_NULL: Value of Nothing/NULL was specified for a property " +
			"that must have a value, such as one " +
			"that is marked by a Key, Indexed, or Not_Null qualifier. (0x80041028)"
		);
		map.put(
			2147749929L,
			"WBEM_E_INVALID_QUALIFIER_TYPE: Variant value for a qualifier was provided that is not a legal qualifier type. (0x80041029)"
		);
		map.put(2147749930L, "WBEM_E_INVALID_PROPERTY_TYPE: CIM type specified for a property is not valid. (0x8004102A)");
		map.put(
			2147749931L,
			"WBEM_E_VALUE_OUT_OF_RANGE: Request was made with an out-of-range value or it is incompatible with the type. (0x8004102B)"
		);
		map.put(
			2147749932L,
			"WBEM_E_CANNOT_BE_SINGLETON: Illegal attempt was made to make a class singleton, such as when the class is derived from a non-singleton class. (0x8004102C)"
		);
		...
		map.put(
			2147749935L,
			"WBEM_E_INVALID_METHOD_PARAMETERS: Parameters provided for the method are not valid. (0x8004102F)"
		);
		map.put(
			2147749936L,
			"WBEM_E_SYSTEM_PROPERTY: There was an attempt to get qualifiers on a system property. (0x80041030)"
		);
		...
		map.put(
			...
			" Note that due to the timing and nature of the asynchronous operation, the operation may " +
			"not have been truly canceled. (0x80041032)"
		);
		map.put(
			2147749939L,
			"WBEM_E_SHUTTING_DOWN: User has requested an operation while WMI is in the process of shutting down. (0x80041033)"
		);
		map.put(
			2147749940L,
			"WBEM_E_PROPAGATED_METHOD: Attempt was made to reuse an existing method name from a parent class and the signatures do not match. (0x80041034)"
		);
		map.put(
			2147749941L,
			"WBEM_E_UNSUPPORTED_PARAMETER: One or more parameter values, such as a " +
			"query text, is too complex or unsupported. WMI is therefore requested " +
			"to retry the operation with simpler parameters. (0x80041035)"
		);
		map.put(2147749942L, "WBEM_E_MISSING_PARAMETER_ID: Parameter was missing from the method call. (0x80041036)");
		map.put(
			2147749943L,
			"WBEM_E_INVALID_PARAMETER_ID: Method parameter has an ID qualifier that is not valid. (0x80041037)"
		);
		map.put(
			2147749944L,
			"WBEM_E_NONCONSECUTIVE_PARAMETER_IDS: One or more of the method parameters have ID qualifiers that are out of sequence. (0x80041038)"
		);
		...
		map.put(
			2147749947L,
			"WBEM_E_OUT_OF_DISK_SPACE: Disk is out of space or the 4 GB limit on WMI repository (CIM repository) size is reached. (0x8004103B)"
		);
		map.put(
			2147749948L,
			"WBEM_E_BUFFER_TOO_SMALL: Supplied buffer was too small to hold all of the objects in the enumerator or to read a string property. (0x8004103C)"
		);
		map.put(
			2147749949L,
			"WBEM_E_UNSUPPORTED_PUT_EXTENSION: Provider does not support the requested put operation. (0x8004103D)"
		);
		map.put(
			2147749950L,
			"WBEM_E_UNKNOWN_OBJECT_TYPE: Object with an incorrect type or version was encountered during marshaling. (0x8004103E)"
		);
		map.put(
			2147749951L,
			"WBEM_E_UNKNOWN_PACKET_TYPE: Packet with an incorrect type or version was encountered during marshaling. (0x8004103F)"
		);
		...
		map.put(
			2147749954L,
			"WBEM_E_INVALID_QUALIFIER: Attempt was made to mismatch qualifiers, such as putting [key] on an object instead of a property. (0x80041042)"
		);
		map.put(
			2147749955L,
			"WBEM_E_INVALID_DUPLICATE_PARAMETER: Duplicate parameter was declared in a CIM method. (0x80041043)"
		);
		map.put(2147749956L, "WBEM_E_TOO_MUCH_DATA: Reserved for future use. (0x80041044)");
		map.put(
			2147749957L,
			"WBEM_E_SERVER_TOO_BUSY: Call to IWbemObjectSink::Indicate has failed. The provider can refire the event. (0x80041045)"
		);
		map.put(2147749958L, "WBEM_E_INVALID_FLAVOR: Specified qualifier flavor was not valid. (0x80041046)");
		map.put(
			2147749959L,
			"WBEM_E_CIRCULAR_REFERENCE: Attempt was made to create a reference that is circular (for example, deriving a class from itself). (0x80041047)"
		);
		map.put(2147749960L, "WBEM_E_UNSUPPORTED_CLASS_UPDATE: Specified class is not supported. (0x80041048)");
		map.put(
			2147749961L,
			"WBEM_E_CANNOT_CHANGE_KEY_INHERITANCE: Attempt was made to change a key when instances or subclasses are already using the key. (0x80041049)"
		);
		map.put(
			2147749968L,
			"WBEM_E_CANNOT_CHANGE_INDEX_INHERITANCE: An attempt was made to change an index when instances or subclasses are already using the index. (0x80041050)"
		);
		map.put(
			2147749969L,
			"WBEM_E_TOO_MANY_PROPERTIES: Attempt was made to create more properties than the current version of the class supports. (0x80041051)"
		);
		map.put(
			2147749970L,
			"WBEM_E_UPDATE_TYPE_MISMATCH: Property was redefined with a conflicting type in a derived class. (0x80041052)"
		);
		map.put(
			2147749971L,
			"WBEM_E_UPDATE_OVERRIDE_NOT_ALLOWED: Attempt was made in a derived class to override a qualifier that cannot be overridden. (0x80041053)"
		);
		map.put(
			2147749972L,
			"WBEM_E_UPDATE_PROPAGATED_METHOD: Method was re-declared with a conflicting signature in a derived class. (0x80041054)"
		);
		map.put(
			2147749973L,
			"WBEM_E_METHOD_NOT_IMPLEMENTED: Attempt was made to execute a method not marked with [implemented] in any relevant class. (0x80041055)"
		);
		map.put(
			2147749974L,
			"WBEM_E_METHOD_DISABLED: Attempt was made to execute a method marked with [disabled]. (0x80041056)"
		);
		...
		map.put(
			2147749977L,
			"WBEM_E_NOT_EVENT_CLASS: The FROM clause of a filtering query references a class that is not an event class (not derived from __Event). (0x80041059)"
		);
		map.put(
			2147749978L,
			"WBEM_E_MISSING_GROUP_WITHIN: A GROUP BY clause was used without the corresponding GROUP WITHIN clause. (0x8004105A)"
		);
		map.put(
			2147749979L,
			"WBEM_E_MISSING_AGGREGATION_LIST: A GROUP BY clause was used. Aggregation on all properties is not supported. (0x8004105B)"
		);
		map.put(
			2147749980L,
			"WBEM_E_PROPERTY_NOT_AN_OBJECT: Dot notation was used on a property that is not an embedded object. (0x8004105C)"
		);
		map.put(
			2147749981L,
			"WBEM_E_AGGREGATING_BY_OBJECT: A GROUP BY clause references a property that is an embedded object without using dot notation. (0x8004105D)"
		);
		map.put(
			2147749983L,
			"WBEM_E_UNINTERPRETABLE_PROVIDER_QUERY: Event provider registration " +
			"query (__EventProviderRegistration) did not specify the classes " +
			"for which events were provided. (0x8004105F)"
		);
		map.put(
			2147749984L,
			"WBEM_E_BACKUP_RESTORE_WINMGMT_RUNNING: Request was made to back up " +
			"or restore the repository while it was in use by WinMgmt.exe, " +
			"or by the SVCHOST process that contains the WMI service. (0x80041060)"
		);
		map.put(
			2147749985L,
			"WBEM_E_QUEUE_OVERFLOW: Asynchronous delivery queue overflowed from the event consumer being too slow. (0x80041061)"
		);
		map.put(
			2147749986L,
			"WBEM_E_PRIVILEGE_NOT_HELD: Operation failed because the client did not have the necessary security privilege. (0x80041062)"
		);
		map.put(2147749987L, "WBEM_E_INVALID_OPERATOR: Operator is not valid for this property type. (0x80041063)");
		map.put(
			2147749988L,
			"WBEM_E_LOCAL_CREDENTIALS: User specified a username/password/authority on a local connection. " +
			"The user must use a blank username/password and rely on default security. (0x80041064)"
		);
		map.put(
			2147749989L,
			"WBEM_E_CANNOT_BE_ABSTRACT: Class was made abstract when its parent class is not abstract. (0x80041065)"
		);
		map.put(
			2147749990L,
			"WBEM_E_AMENDED_OBJECT: Amended object was written without the WBEM_FLAG_USE_AMENDED_QUALIFIERS flag being specified. (0x80041066)"
		);
		map.put(
			2147749991L,
			...
			"objects from the enumerator in a timely fashion, causing the enumerator's object caches to back up. (0x80041067)"
		);
		...
		map.put(
			2147749996L,
			"WBEM_E_QUOTA_VIOLATION: WMI is taking up too much memory. This can be caused by low memory availability or excessive memory consumption by WMI. (0x8004106C)"
		);
		...
		map.put(
			2147750003L,
			"WBEM_E_PROPERTY_NAME_TOO_WIDE: Property name contains more than 255 characters. (0x80041073)"
		);
		...
		map.put(
			2147750006L,
			"WBEM_E_QUALIFIER_NAME_TOO_WIDE: Qualifier name contains more than 255 characters. (0x80041076)"
		);
		map.put(
			2147750007L,
			"WBEM_E_RERUN_COMMAND: The SQL command must be rerun because there is a deadlock in SQL. This can be returned only when data is being stored in an SQL database. (0x80041077)"
		);
		map.put(
			2147750008L,
			"WBEM_E_DATABASE_VER_MISMATCH: The database version does not match the version that the repository driver processes. (0x80041078)"
		);
		map.put(
			2147750009L,
			"WBEM_E_VETO_DELETE: WMI cannot execute the delete operation because the provider does not allow it. (0x80041079)"
		);
		map.put(
			2147750010L,
			"WBEM_E_VETO_PUT: WMI cannot execute the put operation because the provider does not allow it. (0x8004107A)"
		);
		map.put(
			2147750016L,
			"WBEM_E_INVALID_LOCALE: Specified locale identifier was not valid for the operation. (0x80041080)"
		);
		map.put(2147750017L, "WBEM_E_PROVIDER_SUSPENDED: Provider is suspended. (0x80041081)");
		map.put(
			2147750018L,
			"WBEM_E_SYNCHRONIZATION_REQUIRED: Object must be written to the WMI repository and retrieved again before the requested operation can succeed." +
			" This constant is returned when an object must be committed and retrieved to see the property value. (0x80041082)"
		);
		map.put(2147750019L, "WBEM_E_NO_SCHEMA: Operation cannot be completed; no schema is available. (0x80041083)");
		map.put(
			2147750020L,
			"WBEM_E_PROVIDER_ALREADY_REGISTERED: Provider cannot be registered because it is already registered. (0x119FD010)"
		);
		...
		map.put(
			2147750023L,
			"WBEM_E_ENCRYPTED_CONNECTION_REQUIRED: User attempted to set a computer name or domain without an encrypted connection. (0x80041087)"
		);
		map.put(
			2147750024L,
			"WBEM_E_PROVIDER_TIMED_OUT: A provider failed to report results within the specified timeout. (0x80041088)"
		);
		map.put(2147750025L, "WBEM_E_NO_KEY: User attempted to put an instance with no defined key. (0x80041089)");
		map.put(
			2147750026L,
			"WBEM_E_PROVIDER_DISABLED: User attempted to register a provider instance but the COM server for the provider instance was unloaded. (0x8004108A)"
		);
		map.put(
			2147753985L,
			"WBEMESS_E_REGISTRATION_TOO_BROAD: Provider registration overlaps with the system event domain. (0x80042001)"
		);
		map.put(
			2147753986L,
			"WBEMESS_E_REGISTRATION_TOO_PRECISE: A WITHIN clause was not used in this query. (0x80042002)"
		);
		map.put(
			2147753987L,
			"WBEMESS_E_AUTHZ_NOT_PRIVILEGED: This computer does not have the necessary domain permissions " +
			"to support the security functions that relate to the created subscription instance. Contact the " +
			"Domain Administrator to get this computer added to the Windows Authorization Access Group. (0x80042003)"
		);
		...
		map.put(
			2147762180L,
			"WBEMMOF_E_EXPECTED_CLOSE_BRACE: Missing closing brace or an illegal array element (0x80044004)"
		);
		...
		map.put(
			2147762183L,
			"WBEMMOF_E_ILLEGAL_CONSTANT_VALUE: Numeric value out of range or strings without quotes. (0x80044007)"
		);
		...
		map.put(
			2147762189L,
			"WBEMMOF_E_TYPEDEF_NOT_SUPPORTED: Typedefs and enumerated types are not supported. (0x8004400D)"
		);
		map.put(
			2147762190L,
			"WBEMMOF_E_UNEXPECTED_ALIAS: Only a reference to a class object can have an alias value. (0x8004400E)"
		);
		map.put(
			2147762191L,
			"WBEMMOF_E_UNEXPECTED_ARRAY_INIT: Unexpected array initialization. Arrays must be declared with []. (0x8004400F)"
		);
		...
		map.put(
			2147762196L,
			"WBEMMOF_E_EXPECTED_CLASS_NAME: Unexpected character in class name must be an identifier. (0x80044014)"
		);
		map.put(
			2147762197L,
			"WBEMMOF_E_TYPE_MISMATCH: The value specified cannot be made into the appropriate type. (0x80044015)"
		);
		map.put(
			2147762198L,
			"WBEMMOF_E_EXPECTED_ALIAS_NAME: Dollar sign must be followed by an alias name as an identifier. (0x80044016)"
		);
		map.put(2147762199L, "WBEMMOF_E_INVALID_CLASS_DECLARATION: Class declaration is not valid. (0x80044017)");
		map.put(
			2147762200L,
			"WBEMMOF_E_INVALID_INSTANCE_DECLARATION: The instance declaration is not valid. It must start with \"instance of\". (0x80044018)"
		);
		map.put(
			2147762201L,
			"WBEMMOF_E_EXPECTED_DOLLAR: Expected dollar sign. An alias in the form \"$name\" must follow the \"as\" keyword. (0x80044019)"
		);
		map.put(
			2147762202L,
			"WBEMMOF_E_CIMTYPE_QUALIFIER: \"CIMTYPE\" qualifier cannot be specified directly in a MOF file. Use standard type notation. (0x8004401A)"
		);
		map.put(2147762203L, "WBEMMOF_E_DUPLICATE_PROPERTY: Duplicate property name was found in the MOF. (0x8004401B)");
		map.put(
			2147762204L,
			"WBEMMOF_E_INVALID_NAMESPACE_SPECIFICATION: Namespace syntax is not valid. References to other servers are not allowed. (0x8004401C)"
		);
		map.put(2147762205L, "WBEMMOF_E_OUT_OF_RANGE: Value out of range. (0x8004401D)");
		map.put(
			2147762206L,
			"WBEMMOF_E_INVALID_FILE: The file is not a valid text MOF file or binary MOF file. (0x8004401E)"
		);
		...
		map.put(
			2147762209L,
			"WBEMMOF_E_DUPLICATE_QUALIFIER: Qualifier was used more than once on the object. (0x80044021)"
		);
		map.put(
			2147762210L,
			"WBEMMOF_E_EXPECTED_FLAVOR_TYPE: Expected a flavor type such as ToInstance, ToSubClass, EnableOverride, or DisableOverride. (0x80044022)"
		);
		map.put(
			2147762211L,
			"WBEMMOF_E_INCOMPATIBLE_FLAVOR_TYPES: Combining EnableOverride and DisableOverride on same qualifier is not legal. (0x80044023)"
		);
		map.put(2147762212L, "WBEMMOF_E_MULTIPLE_ALIASES: An alias cannot be used twice. (0x80044024)");
		map.put(
			2147762213L,
			"WBEMMOF_E_INCOMPATIBLE_FLAVOR_TYPES2: Combining Restricted, and ToInstance or ToSubClass is not legal. (0x80044025)"
		);
		...
		map.put(
			2147762217L,
			"WBEMMOF_E_EXPECTED_BRACE_OR_BAD_TYPE: The final brace and semi-colon for a class are missing. (0x80044029)"
		);
		map.put(
			2147762218L,
			"WBEMMOF_E_UNSUPPORTED_CIMV22_QUAL_VALUE: A CIM version 2.2 feature is not supported for a qualifier value. (0x8004402A)"
		);
		map.put(
			2147762219L,
			"WBEMMOF_E_UNSUPPORTED_CIMV22_DATA_TYPE: The CIM version 2.2 data type is not supported. (0x8004402B)"
		);
		map.put(
			2147762220L,
			"WBEMMOF_E_INVALID_DELETEINSTANCE_SYNTAX: The delete instance syntax is not valid. It should be #pragma DeleteInstance(\"instancepath\", FAIL|NOFAIL). (0x8004402C)"
		);
		map.put(
			2147762221L,
			"WBEMMOF_E_INVALID_QUALIFIER_SYNTAX: The qualifier syntax is not valid. It should be qualifiername:type=value,scope(class|instance), flavorname. (0x8004402D)"
		);
		map.put(
			2147762222L,
			"WBEMMOF_E_QUALIFIER_USED_OUTSIDE_SCOPE: The qualifier is used outside of its scope. (0x8004402E)"
		);
		map.put(
			2147762223L,
			"WBEMMOF_E_ERROR_CREATING_TEMP_FILE: Error creating temporary file. The temporary file is an intermediate stage in the MOF compilation. (0x8004402F)"
		);
		map.put(
			2147762224L,
			"WBEMMOF_E_ERROR_INVALID_INCLUDE_FILE: A file included in the MOF by the preprocessor command #include is not valid. (0x80044030)"
		);
		map.put(
			2147762225L,
			"WBEMMOF_E_INVALID_DELETECLASS_SYNTAX: The syntax for the preprocessor commands #pragma deleteinstance or #pragma deleteclass is not valid. (0x80044031)"
		);
	...
	private WmiWbemServices(
		...
		this.hostname =
			(networkResource != null && networkResource.startsWith("\\\\")) ? networkResource.split("\\\\")[2] : null;
	 ...
	 * @throws WmiComException For any problem encountered with JNA
	 */
	public static WmiWbemServices getInstance(final String networkResource, final String username, final char[] password)
	...
	public List<Map<String, Object>> executeWql(final WqlQuery wqlQuery, final long timeout)
		...
		try (AutoCloseableReadWriteLock.AutoCloseableReadLock readLock = lock.read()) {
			...
			try {
				...
				return processWqlResult(wbemClassObjectQueryResult, authIdent, timeout, wqlQuery.getSubPropertiesMap());
			} catch (final COMException e) {
	 ...
	 */
	static void setProxySecurity(final Pointer pProxy, final CoAuthIdentity authIdent) throws WmiComException {
	...
	static IWbemContext createWbemContextFor64BitWbemProvider() {
		...
		try {
			...
			return COMUtils.FAILED(hResult) ? null : new IWbemContext(contextPointer.getValue()); // Return the WBEM Context object
		} finally {
	...
	private static Map<String, Set<String>> normalizeProperties(
		...
		final Map<String, String> propertiesNames = Stream
			.of(names)
			.collect(Collectors.toMap(String::toLowerCase, Function.identity()));

		return properties == null || properties.isEmpty()
			? propertiesNames.values().stream().collect(Collectors.toMap(Function.identity(), p -> Collections.emptySet()))
			: properties // Put the real property name from the class
				.entrySet()
				.stream()
				.collect(
					HashMap::new,
					(map, entry) -> map.put(propertiesNames.get(entry.getKey().toLowerCase()), entry.getValue()),
					HashMap::putAll
				);
	}
		...
		try (AutoCloseableReadWriteLock.AutoCloseableReadLock readLock = lock.read()) {
			...
			try {
				final HRESULT hResult = (HRESULT) WmiComHelper.comInvokerInvokeNativeObject(
					...
					new Object[] { pWbemServices, strObjectPath, lFlags, pCtx, ppObject, ppCallResult },
					HRESULT.class
				);
		...
		try (AutoCloseableReadWriteLock.AutoCloseableReadLock readLock = lock.read()) {
			...
			inputMap.forEach((name, value) -> {
				...
				if (value == null) {
					...
					variant = new VARIANT((int) value);
				} else if (value instanceof Pointer) {
					variant = new VARIANT((Pointer) value);
				} else {
	 ...
	 * @param pInParams May be NULL if no in-parameters are required to execute the method.
	 * Otherwise, this points to an IWbemClassObject that contains the properties acting as
	 * inbound parameters for the method execution.
	 * @return A pointer on Execution method output.
	 *
	 * @throws WmiComException For any problem encountered with JNA.
	 */
	Pointer executeMethod(final String objectPath, final String methodName, final Pointer pInParams)
		throws WmiComException {
		Utils.checkNonNull(objectPath, "objectPath");
	 ...
	 * @param pVal A parameter that must point to a valid VARIANT, which becomes the new property value.
	 * If pVal is NULL or points to a VARIANT of type VT_NULL, the property is set to NULL, that is, no value.
	 *
	 * @throws WmiComException For any problem encountered with JNA.
	 */
	public void objectPut(final Pointer pWbemClassObject, final String propertyName, final Variant.VARIANT pVal)
		throws WmiComException {
		try (AutoCloseableReadWriteLock.AutoCloseableReadLock readLock = lock.read()) {
			checkState();
	...
	public Pointer spawnInstance(final Pointer pWbemClassObject) throws WmiComException {
		try (AutoCloseableReadWriteLock.AutoCloseableReadLock readLock = lock.read()) {
			...
			final HRESULT hResult = (HRESULT) WmiComHelper.comInvokerInvokeNativeObject(
				...
				new Object[] { pWbemClassObject, lFlags, ppNewInstance },
				HRESULT.class
			);
	 ...
	 * @throws WmiComException For any problem encountered with JNA.
	 */
	Pointer getMethod(final Pointer pWbemClassObject, final String methodName) throws WmiComException {
		Utils.checkNonNull(pWbemClassObject, "pWbemClassObject");
		Utils.checkNonNull(methodName, "methodName");
		...
		final HRESULT hResult = (HRESULT) WmiComHelper.comInvokerInvokeNativeObject(
			...
			new Object[] { pWbemClassObject, wszName, lFlags, ppInSignature, ppOutSignature },
			HRESULT.class
		);
