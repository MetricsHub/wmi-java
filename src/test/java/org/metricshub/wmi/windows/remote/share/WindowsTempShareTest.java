...
class WindowsTempShareTest {
	...
	static void initMockedUtils() throws Exception {
		...
		MOCKED_UTILS
			.when(() -> Utils.checkArgumentNotZeroOrNegative(Mockito.anyLong(), Mockito.anyString()))
			.thenCallRealMethod();
		...
		MOCKED_UTILS.when(() -> Utils.checkNonBlank(Mockito.anyString(), Mockito.anyString())).thenCallRealMethod();
	...
	void testWindowsTempShare() {
		...
		// Check arguments
		Assertions.assertThrows(IllegalArgumentException.class, () -> new WindowsTempShare(null, "share", "remote"));
		...
		{
			final WindowsTempShare windowsTempShare = new WindowsTempShare(
		...
		}
		{
			...
			final WindowsTempShare windowsTempShare = new WindowsTempShare(windowsRemoteExecutor, "share", null);
		...
		}
		{
			...
			final WindowsTempShare windowsTempShare = new WindowsTempShare(windowsRemoteExecutor, "share", "remote");
	...
	void testGetOrCreateShare() throws Exception {
		...
		final ShareRemoteDirectoryConsumer<WindowsRemoteExecutor, String, String, Long> shareRemoteDirectory = Mockito.mock(
			ShareRemoteDirectoryConsumer.class
		);
		...
		try (final MockedStatic<WindowsTempShare> mockedWindowsTempShare = Mockito.mockStatic(WindowsTempShare.class)) {
			mockedWindowsTempShare
				.when(() -> WindowsTempShare.getClusterShare(Mockito.eq(windowsRemoteExecutor), Mockito.anyLong()))
				.thenReturn(Optional.of(windowsTempShare));

			mockedWindowsTempShare
				.when(() -> WindowsTempShare.getOrCreateShare(windowsRemoteExecutor, timeout, shareRemoteDirectory))
				.thenCallRealMethod();

			mockedWindowsTempShare.verify(WindowsTempShare::buildShareName, Mockito.times(0));
			mockedWindowsTempShare.verify(
				() ->
					WindowsTempShare.getShare(Mockito.any(WindowsRemoteExecutor.class), Mockito.anyString(), Mockito.anyLong()),
				Mockito.times(0)
			);
			mockedWindowsTempShare.verify(
				() ->
			...
			final WindowsTempShare actual = WindowsTempShare.getOrCreateShare(
				windowsRemoteExecutor,
				timeout,
		...
		try (final MockedStatic<WindowsTempShare> mockedWindowsTempShare = Mockito.mockStatic(WindowsTempShare.class)) {
			...
			mockedWindowsTempShare
				.when(() -> WindowsTempShare.getShare(Mockito.eq(windowsRemoteExecutor), Mockito.anyString(), Mockito.anyLong())
				)
				.thenReturn(Optional.of(windowsTempShare));
	...
	void testGetWindowsDirectory() throws Exception {
		...
		// Check arguments
		Assertions.assertThrows(IllegalArgumentException.class, () -> WindowsTempShare.getWindowsDirectory(null, timeout));
		...
		// case invalid query syntax (impossible in real case)
		{
			final WindowsRemoteExecutor windowsRemoteExecutor = Mockito.mock(WindowsRemoteExecutor.class);
			Mockito
				.doThrow(WqlQuerySyntaxException.class)
				.when(windowsRemoteExecutor)
				.executeWql(Mockito.anyString(), Mockito.anyLong());
		...
		// case query without result
		{
			...
			Mockito
				...
				.when(windowsRemoteExecutor)
				.executeWql(Mockito.eq("SELECT WindowsDirectory FROM Win32_OperatingSystem"), Mockito.anyLong());
		...
		// case query with result without WindowsDirectory field (impossible in real case)
		{
			final WindowsRemoteExecutor windowsRemoteExecutor = Mockito.mock(WindowsRemoteExecutor.class);
			Mockito
				.doReturn(Collections.singletonList(Collections.singletonMap("key", "value")))
				.when(windowsRemoteExecutor)
				.executeWql(Mockito.eq("SELECT WindowsDirectory FROM Win32_OperatingSystem"), Mockito.anyLong());
		...
		// Windows directory found
		{
			...
			final WindowsRemoteExecutor windowsRemoteExecutor = Mockito.mock(WindowsRemoteExecutor.class);
			Mockito
				.doReturn(Collections.singletonList(Collections.singletonMap("WindowsDirectory", winDirectory)))
				.when(windowsRemoteExecutor)
				.executeWql(Mockito.eq("SELECT WindowsDirectory FROM Win32_OperatingSystem"), Mockito.anyLong());

			Assertions.assertEquals(winDirectory, WindowsTempShare.getWindowsDirectory(windowsRemoteExecutor, timeout));
		}
	}
	...
	void testCreateRemoteDirectory() throws Exception {
		...
		Mockito
			.doReturn(new WindowsRemoteCommandResult("stdout", "stderr", 1.0f, 0))
			.when(windowsRemoteExecutor)
			.executeCommand(Mockito.anyString(), Mockito.isNull(), Mockito.isNull(), Mockito.anyLong());
	...
	void testBuildRemotePath() {
		...
		Assertions.assertThrows(IllegalArgumentException.class, () -> WindowsTempShare.buildRemotePath("Windows", null));

		Assertions.assertThrows(IllegalArgumentException.class, () -> WindowsTempShare.buildRemotePath("Windows", ""));

		Assertions.assertThrows(IllegalArgumentException.class, () -> WindowsTempShare.buildRemotePath("Windows", " "));

		Assertions.assertEquals("\\Temp\\SEN_ShareFor_TEST$", WindowsTempShare.buildRemotePath("", "SEN_ShareFor_TEST$"));
	...
	void tesBuildPathOnCluster() {
		...
		Assertions.assertEquals("\\path\\Temp\\SEN_TempFor_PC-TEST", WindowsTempShare.buildPathOnCluster("\\path"));
	}
